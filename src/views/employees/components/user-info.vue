...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      ...
      this.$refs.staffPhotos.list = [{ url: this.formData.staffPhoto, aa: true }]
    },
    async savePersonal() {
      const fileList = this.$refs.staffPhotos.list
      if (fileList.some(item => !item.aa)) {
    ...
    async saveUser() {
      //  调用父组件
      // 去读取 员工上传的头像
      const fileList = this.$refs.staffPhoto.list // 读取上传组件的数据
      if (fileList.some(item => !item.aa)) {
        //  如果此时去找 upload为false的图片 找到了说明 有图片还没有上传完成
        this.$message.warning('您当前还有图片没有上传完成！')
        return
    ...
    async getUserDetailById() {
      ...
      this.$refs.staffPhoto.list = [{ url: this.userInfo.staffPhoto, aa: true }]
    }
  }
}
